...
def draw_input_box(input_text):
    ...
    pygame.draw.rect(screen, BLUE, (600, 600, 300, 40), 2)  # Dessiner
    text_surface = font.render(input_text, True, BLACK)
    screen.blit(text_surface, (605, 605))  # Afficher le texte 
...
#fonctions d'ajout au dictionnaire
...
class Button:
    ...
    #Action de survolement et de click
    def is_hovered(self):
...
def quit_game():
    ...
    pygame.quit()
    sys.exit()
#fonction permettant l'accès aux différents menus
def deck1():
    """Retourner Deck 1"""
    return 'deck1'
...
def main_menu():
    ...
    while running:
        ...
        title_text = font.render("CLASH OF LIFE", True, (0, 0, 0))  # Titre
       ...
       # Affichage du message d'erreur si les decks sont vides
        if show_error:
...
def afficher_selection(selected_ships, y_position):
    ...
    for i in range(num_zones):
        ...
        pygame.draw.rect(screen, zone_color, (x_pos, y_position, zone_width, zone_height))  
        pygame.draw.rect(screen, border_color, (x_pos, y_position, zone_width, zone_height), 2)  
...
# Menu pour Deck 2 ( similaire au menu du deck 1 avec d2 et en rouge)
def Menu_deck2():
...
def Menu_param():
    ...
    while running:
        ...
        buttons = []
        for i, (label, (width, height)) in enumerate(options.items()):  #on met les options de taille 
